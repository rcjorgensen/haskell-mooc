...
firstHalf s = let l = length s
                  h = if even l then div l 2 else div (l+1) 2
              in take h s 
...
joinToLength n xs = filter (\x -> length x == n) [ a ++ b | a <- xs, b <- xs ]
...
sumRights []     = 0
sumRights (x:xs) = either (const 0) id x + sumRights xs
...
interpreter [] = []
interpreter cs = interpreter' 0 0 cs
