...
totalPrice (MkShoppingEntry name price amount) = price * (fromIntegral amount)
...
data Position = MkPosition { x :: Int, y :: Int } deriving Show
...
tick :: UpDown -> UpDown
tick (Increasing i) = Increasing (i + 1)
tick (Decreasing i) = Decreasing (i - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (Increasing i) = Decreasing i
toggle (Decreasing i) = Increasing i
...
rgb Green = [0,1,0]
rgb Blue  = [0,0,1]
rgb (Mix x y) = averageLists (rgb x) (rgb y)
rgb (Invert x) = subtractLists [1,1,1] (rgb x)

averageLists :: [Double] -> [Double] -> [Double]
averageLists (x:xs) (y:ys) = (x + y) / 2 : averageLists xs ys
averageLists _ _           = []
